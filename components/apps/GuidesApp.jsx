...
import { Plus, Trash2, Loader2, X, FileText, ChevronDown, ChevronUp, Video, Link2, Bold, Italic, Image as ImageIcon, Heading2, MessageSquare, Newspaper, FileQuestion, UploadCloud, LayoutList, GripVertical } from 'lucide-react';

export default function GuidesApp({ session, unaData, activeTab }) {
    ...
    const [activeGuide, setActiveGuide] = useState(null); // When set, views a specific guide

    const [showModal, setShowModal] = useState(false);
    const [isSaving, setIsSaving] = useState(false);
    
    // Default structure for the editing modal
    const [editingGuide, setEditingGuide] = useState({
        id: null,
        title: '',
        category_id: '',
        type: 'article', // 'article' or 'faq'
        content: ''
    });

    // Tracking the drag state for FAQs
    ...
    // Resets view to list when tab changes
    useEffect(() => {
        setActiveGuide(null);
    }, [activeTab]);
    ...
    const handleSaveGuide = async () => {
        ...
        setIsSaving(true);
        try {
            // content is either string (HTML) or array of objects (FAQ). Serialize it to JSON string.
            const payload = {
                ...editingGuide,
    ...
    const insertTag = (prefix, suffix) => {
        ...
        // Return focus to textarea and set cursor position after render
        setTimeout(() => {
            textarea.focus();
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4 sm:gap-6">
                <div>
                    <h2 className="text-3xl md:text-5xl font-black uppercase italic tracking-tighter leading-none mb-2 md:mb-4 text-white">
                        {activeCategory ? activeCategory.name : 'Help Center'}
                    </h2>
                            ...
                            <p className="text-xs text-gray-500 mt-auto pt-4 flex items-center justify-between">
                                ...
                                {isAdmin && (
                                    <span className="flex items-center gap-2">
                                        <button onClick={(e) => { e.stopPropagation(); setEditingGuide({...guide, content: JSON.parse(guide.content)}); setShowModal(true); }} className="text-gray-500 hover:text-white transition-colors p-1"><Edit size={14}/></button>
                                        <button onClick={(e) => { e.stopPropagation(); handleDeleteGuide(guide.id); }} className="text-gray-500 hover:text-red-500 transition-colors p-1"><Trash2 size={14}/></button>
                                    </span>
                                )}
                            </p>
                        </div>
...
// Re-usable mock for admin edit icon
const Edit = ({ size }) => (
