...
import { ChevronsUpDown, LogOut, CreditCard, LayoutDashboard, Image, Link2, FileText, Users } from 'lucide-react';
                ...
                {isAppSwitcherOpen && (
                    <>
                        ...
                        <div className="absolute top-full left-0 mt-2 w-64 bg-[#111] rounded-2xl shadow-2xl border border-white/10 overflow-hidden z-50 py-2 animate-in slide-in-from-top-2 duration-200">
                            ...
                            <button onClick={() => handleAppSwitch('address-book', 'contacts')} className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-bold transition-colors ${currentApp === 'address-book' ? 'bg-white/10 text-white' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}><Users size={18} className={currentApp === 'address-book' ? 'text-[#9df01c]' : ''}/> Address Book</button>
