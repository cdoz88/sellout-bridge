...
// --- 1. SETTINGS (Sender Name, Reply-To, Socials, Branding) ---
...
router.get('/api/newsletter/lists', async (req, res) => {
    try {
        ...
        for (let list of lists) {
            ...
            list.subscriber_count = parseInt(subs[0].count);
        }
        res.json({ lists });
    } catch (err) { res.status(500).json({ error: "Failed to fetch lists." }); }
});

router.post('/api/newsletter/lists', async (req, res) => {
    try {
        const user = await getAuthenticatedUser(req.headers.authorization);
        if (!user) return res.status(401).json({ error: "Not authenticated" });
...
router.post('/api/newsletter/lists/delete', async (req, res) => {
    try {
        const user = await getAuthenticatedUser(req.headers.authorization);
        if (!user) return res.status(401).json({ error: "Not authenticated" });
...
router.post('/api/newsletter/lists/import', async (req, res) => {
    try {
        ...
        for (const sub of subscribers) {
            ...
            const fName = (sub.firstName || '').trim();
...
router.post('/api/newsletter/send', async (req, res) => {
    try {
        ...
        for (const recipient of validRecipients) {
            ...
            // --- THE MERGE TAG ENGINE ---
            if (finalHtml.includes('{first_name}')) {
                const nameToInsert = recipient.first_name ? recipient.first_name : 'there';
                finalHtml = finalHtml.replace(/{first_name}/gi, nameToInsert);
            }
    ...
    try {
        ...
        const oldCampaigns = await sql`
            ...
            WHERE (status = 'sent' AND sent_at < NOW() - INTERVAL '30 days')
               OR (status = 'draft' AND created_at < NOW() - INTERVAL '30 days')
        `;

        for (const camp of oldCampaigns) {
            if (camp.content) {
                try {
                    const blocks = typeof camp.content === 'string' ? JSON.parse(camp.content) : camp.content;
                    for (const block of blocks) {
                        if (block.type === 'image' && block.url) {
                            await fetch(`https://api.fytsolutions.com/api.php?action=delete_file&fileUrl=${encodeURIComponent(block.url)}`);
                        }
                    }
                } catch(e) {}
            }
